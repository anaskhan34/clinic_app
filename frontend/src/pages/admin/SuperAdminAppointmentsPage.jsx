...
export default function SuperAdminAppointmentsPage() {
  ...
  const statusColors = {
    ...
    IN_PROGRESS: "bg-violet-50 text-violet-750 text-violet-700",
  };
                ...
                {appointments.map((appt) => (
                  <tr key={appt._id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="py-4">
                      ...
                      <p className="text-xs text-slate-450">{appt.patientId?.email}</p>
                    </td>
                    <td className="py-4">
                      <p className="text-sm font-semibold text-slate-800">{appt.doctorId?.name}</p>
                      <p className="text-xs text-slate-500">{appt.clinicId?.name}</p>
                    </td>
                    <td className="py-4">
                      <p className="text-sm font-semibold text-slate-800">{appt.appointmentDate}</p>
                      <p className="text-xs text-slate-450">{appt.appointmentTime}</p>
                    </td>
                    ...
                    <td className="py-4 text-right">
                      <div className="flex gap-2 justify-end items-center">
                        ...
                        <button
                          ...
                          disabled={actionLoadingId === appt._id}
                          className="rounded border border-slate-200 px-2 py-1 text-xs font-semibold text-slate-505 text-slate-500 hover:bg-slate-50 transition hover:text-slate-800"
                        >
                          Delete
                        </button>
                      </div>
